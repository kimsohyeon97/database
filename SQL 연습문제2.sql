...
`a_balance` tinyint not null default 0,
...
insert into `bank_customer` values ('910912-2000005','신사임당',1,'010-1234-1006','강원도 강릉시');
insert into `bank_customer` values ('941127-1000006','이순신',1,'010-1234-1007','부산시 영도구');
...
insert into `bank_account` values('101-22-1006','L1','예금담보대출','730423-1000001',25000,'2013-12-11');
insert into `bank_account` values('101-23-1007','L1','주택담보대출','750210-1000002',2700000,'2020-09-23');

insert into `bank_transaction` (`t_a_no`,`t_dist`,`t_amount`,`t_datetime`) values
('101-11-1001',1,50000,'2020-08-21 04:26:52'),
...
#실습 2-10
select * from `bank_account` where `a_item_name`='s1' and `a_balance`>=1000000;
...
#실습 2-16
select max(`t_amount`) as `큰 금액`, min(`t_amount`) as `작은 금액` from `bank_transaction` where `t-dist`=2;

#실습 2-17
select * from `bank_account` where `a_item_name`='s1' order by `a_balance` desc;
